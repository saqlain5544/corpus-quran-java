// Package token provides the shared Quran text tokenizer used by both
// the pipeline (data preprocessing) and the runtime server (DB loading).
// Keeping a single copy guarantees consistent tokenization across all
// code paths.
package token
...
// BISMILLAH_TEXT is the canonical Bismillah in Uthmani script.  The
// shadda (U+0651) must precede the fatha (U+064E) — matching the
// exact combining-character order used in the Tanzil XML source.
...
// TrailingMarks are the Quran annotation characters that may be
// appended to a word in the source. They match the inline
// Arabic-Extended characters found by the Research Agent.
//
//	U+06D6 .. U+06DB → pause marks
//	U+06DC           → sakta (rare brief pause)
//	U+06DE           → rub' / hizb
//	U+06E2           → small high meem (iqlab mark above noon)
//	U+06ED           → small low meem (iqlab variant below baseline)
//	U+06E9           → sajdah
//
// Other annotation characters (U+06DF, U+06E0, U+06E1, U+06E5..U+06E8,
// U+06EA..U+06EC) are diacritics that belong to the letters themselves;
// they are NOT trailing marks.
var TrailingMarks = map[rune]bool{
	'\u06D6': true,
	'\u06D7': true,
	'\u06D8': true,
	'\u06D9': true,
	...
	'\u06DC': true,
	'\u06DE': true,
	'\u06E2': true, // iqlab (high meem)
	'\u06ED': true, // iqlab (low meem)
	'\u06E9': true,
}
...
func SplitMark(token string) (string, []string) {
	...
	for cut > 0 && TrailingMarks[runes[cut-1]] {
		cut--
...
// whitespace-delimited position contains only marks.
//
